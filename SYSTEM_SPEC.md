...
| Actuator | `health`, `info`, `metrics`, `prometheus` exposed — `/actuator/health` used by Azure App Service health probe · `/actuator/prometheus` for Grafana/Prometheus scraping |
| Docker | Multi-stage build — JDK 25 builder + JRE 25 runtime, ~150MB final image, no source code in runtime image |
...
All three services use the same multi-stage Dockerfile pattern.
Replace `{PORT}` with the service port (8081, 8082, 8083).

```dockerfile
# Stage 1 — Build (JDK + Maven — throwaway after build)
...
# Application Insights agent for OpenTelemetry → Azure Monitor
...
**Why multi-stage:**
- Final image ~150MB vs ~700MB single-stage
- No JDK, no Maven, no source code in the deployed image
- Dependency layer cached — rebuilds skip `dependency:go-offline` if `pom.xml` unchanged
- Same pattern for all three services — consistent and predictable
