...
- [ ] PostgreSQL on port 5432, schemas `users` and `journal`
- [ ] Kafka on port 9092, Redis on port 6379
...
**Implementation order:** pom.xml → Flyway V1-V4 → entities → repos → DTOs → services → controllers → SecurityConfig → Kafka producers → unit tests → Level 2 Cucumber tests

**Definition of Done:**
- [ ] `mvn clean package` passes, all unit tests pass
...
**Implementation order:** pom.xml → Flyway V1-V5 → entities → repos → DTOs → services → controllers → SecurityConfig → Kafka producer + deactivation consumer → export service → unit tests → Level 2 Cucumber tests

**Definition of Done:**
- [ ] `mvn clean package` passes, all tests pass
