...
> **Critical:** Flyway connects as `journal_svc` (not `postgres`). By default `journal_svc`
> only has `USAGE` on the `journal` schema — not `CREATE`. Without `GRANT CREATE ON SCHEMA journal TO journal_svc`
> in `infra/init-schemas.sql`, V1 will fail immediately with:
> `ERROR: permission denied for schema journal`
>
