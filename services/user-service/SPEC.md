...
- `PUT /api/users/password-reset/confirm
    Security: use `MessageDigest.isEqual()` (constant-time comparison) NOT `String.equals()`
              when comparing the submitted token hash against the stored hash.
              String.equals() is vulnerable to timing attacks — attacker measures
              response time differences to brute-force tokens byte by byte.` — `newPassword` field
...
GET  /api/users/me
     Returns: 200 { id, name, email, role }
...
```
> V4 should be added during Phase 4 (Notification Service) — not Phase 2 (User Service implementation).
  The preference columns are consumed by the notification scheduler, not the user service core auth flow.
...
- [ ] Change password returns 400 for wrong current password
...
| 401 | `/errors/invalid-credentials` | Wrong email or password |
| 403 | `/errors/forbidden` | Valid JWT but wrong role |
| 404 | `/errors/not-found` | User not found |
| 409 | `/errors/email-already-exists` | Duplicate registration |
| 429 | `/errors/rate-limit-exceeded` | Too many requests (Phase 8) |
